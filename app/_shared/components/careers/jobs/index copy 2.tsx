...
interface Job {
  ...
  details: Details[];
  category?: string; // Add category to Job interface
  location?: string; // Add location to Job interface
}
...
const Jobs = () => {
  ...
  useEffect(() => {
    filterJobs();
  }, [selectedCategory, selectedExperience, selectedLocation, searchTerm]);
  ...
  const filterJobs = () => {
    let filtered = jobsData.filter((job) => {
      ...
      const matchesExperience =
        selectedExperience === "all" || job.level === selectedExperience;
    ...
    // @ts-ignore
    if (selectedJob && !filtered.includes(selectedJob)) {
  ...
  const experienceOptions = [
    ...
    { title: "Entry Level (0-1 years)", value: "Entry Level" },
    { title: "Mid Level (2-5 years)", value: "Mid-Level" },
    { title: "Senior Level (6+ years)", value: "Senior Level" },
  ];
